...
function switchAITab(tab) {
  ...
  document
    .querySelector(`.ai-tab-btn[data-tab="${tab}"]`)
    .classList.add("active");
  document.querySelector(`.ai-tab-btn[data-tab="${tab}"]`).style.color =
    "var(--primary)";
  document.querySelector(
    `.ai-tab-btn[data-tab="${tab}"]`
  ).style.borderBottomColor = "var(--primary)";

  // Show/hide tab content
  document.getElementById("ai-tab-paste").style.display =
    tab === "paste" ? "block" : "none";
  document.getElementById("ai-tab-upload").style.display =
            ...
            <div style="background: #fff; border-radius: 20px; width: 95%; max-width: 1200px; margin: 30px auto; max-height: 90vh; overflow-y: auto;">
                ...
                <div style="padding: 24px;">
                    <p style="margin-bottom: 20px; color: #64748b;">${
                      questions.length
                    } soal berhasil diekstrak. Edit jika perlu, lalu klik "Tambah ke Ujian".</p>
                          ...
                          .map(
                            ...
                            <div class="preview-question-card" data-index="${idx}" style="background: #f8fafc; border-radius: 12px; padding: 20px; margin-bottom: 20px; border-left: 4px solid var(--primary-light);">
                                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px;">
                                    <h3 style="margin: 0; font-size: 1rem;">Soal ${
                                      idx + 1
                                    } 
                                ...
                                <div class="preview-question-text" style="font-weight: 500; margin-bottom: 16px; color: #1e293b; white-space: pre-wrap; word-break: break-word;">${escapeHtml(
                                  q.text
                                )}</div>
                                          ...
                                          .map((opt, optIdx) => {
                                            ...
                                            if (q.type === "checkbox") {
                                              isCorrect =
                                                ...
                                                q.correct_answers_checkbox.includes(
                                                  optIdx.toString()
                                                );
                                            } else {
                                              isCorrect =
                                                q.correct === optIdx.toString();
                                            }
                                            return `
                                                <div style="display: flex; align-items: center; gap: 10px; margin-bottom: 8px; padding: 8px; background: ${
                                                  isCorrect ? "#d1fae5" : "#fff"
                                                }; border-radius: 8px;">
                                                    <strong>${
                                                      letters[optIdx] ||
                                                      optIdx + 1
                                                    }.</strong>
                                                    <span style="flex:1; white-space: pre-wrap; word-break: break-word;">${escapeHtml(
                                                      opt.text || ""
                                                    )}</span>
                                                    ${
                                                      isCorrect
                                                        ? '<span style="color: #10b981; font-size: 0.75rem;">✓ Benar</span>'
                                                        : ""
                                                    }
                                                </div>
                                            `;
                                ...
                                `
                                    ...
                                    <div style="background: #fff3e0; padding: 12px; border-radius: 8px; margin-top: 8px;">
                                        ...
                                        <p style="margin-top: 8px; font-size: 0.9rem; white-space: pre-wrap; word-break: break-word;">${escapeHtml(
                                          q.correct ||
                ...
                <div style="padding: 16px 24px; border-top: 1px solid #e2e8f0; display: flex; justify-content: space-between; position: sticky; bottom: 0; background: #fff;">
                    ...
                    <button class="btn btn-success" onclick="importQuestionsToExam()">✅ Tambahkan ${
                      questions.length
                    } Soal ke Ujian</button>
                </div>
            </div>
        </div>
    `;
  ...
  if (q.type !== "essay") {
                    ...
                    ${q.options
                      .map(
                        ...
                        <div class="option-row" data-opt-index="${optIdx}">
                            ...
                            <input type="text" class="form-control option-text" value="${escapeHtml(
                              opt.text || ""
                            )}" style="flex: 1;" onchange="updateOptionText(${optIdx}, this.value)">
                      ...
                      q.type === "checkbox"
                        ? q.options
                            ...
                            <label style="display: flex; align-items: center; gap: 8px; margin-bottom: 8px;">
                                <input type="checkbox" value="${optIdx}" ${
                                q.correct_answers_checkbox &&
                                q.correct_answers_checkbox.includes(
                                  optIdx.toString()
                                )
                                  ? "checked"
                                  : ""
                              } onchange="updateCorrectSelection(${optIdx}, this.checked)">
                                <span>${
                                  letters[optIdx] || optIdx + 1
                        ...
                        : q.options
                            .map(
                              (opt, optIdx) => `
                            <label style="display: flex; align-items: center; gap: 8px; margin-bottom: 8px;">
                                <input type="radio" name="edit-correct" value="${optIdx}" ${
                                q.correct === optIdx.toString() ? "checked" : ""
                              } onchange="updateCorrectRadio('${optIdx}')">
                                <span>${
                                  letters[optIdx] || optIdx + 1
                                }. ${escapeHtml(opt.text || "")}</span>
                            </label>
    ...
    optionsHtml = `
            <div class="form-group">
                ...
                <textarea id="edit-essay-answer" class="form-control" rows="4">${escapeHtml(
                  q.correct || ""
                )}</textarea>
            </div>
        `;
            ...
            <div style="background: #fff; border-radius: 16px; width: 90%; max-width: 800px; max-height: 85vh; overflow-y: auto;">
                ...
                <div style="padding: 20px;">
                    <div class="form-group">
                        ...
                        <textarea id="edit-question-text" class="form-control" rows="3">${escapeHtml(
                          q.text
                        )}</textarea>
                    </div>
                    ${optionsHtml}
...
function updateOptionText(optIndex, value) {
  if (currentEditIndex !== null && extractedQuestions[currentEditIndex]) {
    if (!extractedQuestions[currentEditIndex].options[optIndex]) {
      extractedQuestions[currentEditIndex].options[optIndex] = {
        text: "",
        image: null,
      };
    }
...
function addEditOption() {
  if (currentEditIndex !== null && extractedQuestions[currentEditIndex]) {
    ...
    extractedQuestions[currentEditIndex].options.push({
      text: "",
      image: null,
    });
...
function updateCorrectSelection(optIndex, isChecked) {
  if (currentEditIndex !== null && extractedQuestions[currentEditIndex]) {
    ...
    if (isChecked && !q.correct_answers_checkbox.includes(idxStr)) {
      ...
      q.correct_answers_checkbox = q.correct_answers_checkbox.filter(
        (i) => i !== idxStr
      );
    }
  }
  ...
  for (const q of extractedQuestions) {
    ...
    const newQuestion = {
      ...
      correct_answers_checkbox:
        q.type === "checkbox" ? q.correct_answers_checkbox || [] : [],
      points: q.points || 1,
