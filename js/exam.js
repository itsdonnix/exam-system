...
const ExamEngine = {
  ...
  escapeUrl(url) {
    // Basic URL encoding for path segments, not full URLs with schemes/hosts.
    // For full URLs, ensure they are validated to start with http/https or relative paths.
  ...
  async init(examId) {
    ...
    try {
      const response = await fetch(`../php/exam_api.php?action=get_exam&exam_id=${examId}`);
      ...
      this.showResultsSetting = data.exam.show_results_setting || 'direct_submit';
  ...
  renderQuestions() {
    ...
    container.innerHTML = this.questions.map((q, idx) => {
      ...
      return `
        <div class="question-card" id="qcard-${idx}" style="display: ${idx === this.currentQuestion ? 'block' : 'none'}">
          <div class="q-meta">
          ...
          ${(() => {
            let media = [];
            try {
              if (q.media_url && q.media_url.trim() !== "" && q.media_url !== "[]") {
            ...
            if (Array.isArray(media)) {
              media.forEach(url => {
                ...
                if (cleanUrl.length > 5 && (cleanUrl.includes('uploads/') || cleanUrl.startsWith('http'))) {
                  if (!cleanUrl.startsWith('[') && !cleanUrl.endsWith(']')) finalMedia.push(cleanUrl);
                }
              });
            }
            ...
            return `
              <div class="question-media-container" style="display: flex; flex-direction: column; gap: 12px; margin-bottom: 24px; align-items: center;">
                ${finalMedia.map(url => {
                  const finalUrl = this.escapeUrl(url.startsWith('http') ? url : '../' + url);
                  return `
                    <img src="${finalUrl}" alt="Gambar Soal" 
                      onclick="ExamEngine.openZoom('${this.escapeUrl(finalUrl)}')"
          ...
          ${(qType === 'multiple' || qType === 'truefalse' || qType === 'checkbox') ? this.renderOptions(options, idx) : this.renderEssay(q, idx)}
        </div>
      `;
    }).join('');
      ...
      if (qType === 'multiple' || qType === 'truefalse') {
        ...
        document.querySelectorAll(`[name="q${idx}"]`).forEach(cb => {
          cb.addEventListener('change', () => {
            const checked = Array.from(document.querySelectorAll(`[name="q${idx}"]:checked`)).map(c => c.value);
            if (checked.length > 0) {
    ...
    return `<div class="options-list">
      ${options.map((opt, i) => {
        ...
        if (qType === 'multiple') {
          ...
          isChecked = Array.isArray(this.answers[q.id]) && this.answers[q.id].includes(i.toString());
        }
        ...
        return `
          <div class="option-item ${isChecked ? 'selected' : ''}" id="opt-${idx}-${i}" onclick="ExamEngine.selectOption(${idx}, ${i})">
            <input type="${inputType}" name="q${idx}" value="${i}" id="r${idx}-${i}" ${isChecked ? 'checked' : ''} onclick="event.stopPropagation()">
            <div class="opt-letter">${letters[i]}</div>
            <div class="opt-text">
              ${this.escapeHtml(optText)}
              ${optImg ? `
                <div class="option-img-container" style="margin-top:10px" onclick="event.stopPropagation()">
                  <img src="../${this.escapeUrl(optImg)}" style="max-width:100%; max-height:120px; border-radius:8px; cursor:zoom-in;" 
                    onclick="ExamEngine.openZoom('../${this.escapeUrl(optImg)}')">
                </div>
              ` : ''}
            </div>
          </div>
  ...
  goToQuestion(idx) {
    // Hide current
    const currentCard = document.getElementById(`qcard-${this.currentQuestion}`);
  ...
  selectOption(qIdx, optIdx) {
    ...
    if (qType === 'checkbox') {
      ...
      const checked = Array.from(document.querySelectorAll(`[name="q${qIdx}"]:checked`)).map(c => c.value);
      if (checked.length > 0) {
  ...
  async submitExam(forced = false, reason = '') {
    ...
    if (!forced) {
      ...
      const total = this.questions.length;
      if (answered < total) {
        const confirm = window.confirm(`⚠️ Anda baru menjawab ${answered} dari ${total} soal.\nApakah Anda yakin ingin mengumpulkan?`);
        if (!confirm) return;
      }
    }
    ...
    try {
      ...
      if (data.success) {
        ...
        if (this.showResultsSetting === 'never') {
           ...
           if (rTime) rTime.textContent = Math.round(data.time_taken / 60) + ' menit';
