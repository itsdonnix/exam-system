 ...
 * POST: gemini_api_key, gemini_model, csrf_token
 */
...
// CSRF validation - FIXED: Pass both arguments
if (!isset($input['csrf_token']) || !verifyCSRFToken($input['csrf_token'], $_SESSION['csrf_token'])) {
    error_log("[save_ai_settings] CSRF token validation failed for teacher_id: " . $_SESSION['user_id']);
...
try {
    ...
    // Insert or update
    if (empty($apiKey)) {
        // Keep existing API key - only update model
        $stmt = $db->prepare("
