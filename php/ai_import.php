        ...
        if (class_exists('setasign\Fpdi\Fpdi')) {
            ...
            try {
                ...
                for ($i = 1; $i <= $pageCount; $i++) {
                    ...
                    // FIXED: Previously $text .= $text; which causes exponential memory bloat/infinite loop
                    // Note: FPDI is for creating PDFs, it doesn't natively extract text.
                    // This placeholder remains but the logic is corrected to prevent crashes.
                    $text .= " [Page $i] ";
                }
            } catch (Exception $e) {
...
if ($action !== 'extract') {
    echo json_encode(['success' => false, 'message' => 'Invalid action. Use action=extract']);
    exit;
}
...
if (isset($_FILES['file']) && $_FILES['file']['error'] === UPLOAD_ERR_OK) {
    ...
    $inputText = trim($input['text'] ?? $_POST['text'] ?? '');
    $source = 'text';
